...
| Transit 🚌 | Operator GTFS-Realtime VehiclePositions (9 keyless regions, `src/data/transitFeeds.js`; Metro do Porto is timetable-estimated) | `src/layers/transit/` via `src/app/layers/transit.js` | `/api/transit` | 15s (poll + delayed playback) |
...
gate. Nine keyless regions are available: Boston, Austin, Minneapolis–St Paul,
Helsinki, the Netherlands, Norway, South East Queensland and Porto (STCP buses,
NGSI v2 via Porto Digital; Metro do Porto positions estimated from the static
GTFS timetable and labelled as such). Mode silhouettes
